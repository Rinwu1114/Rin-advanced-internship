...
export default  function BookInfo( {bookInfo}: {bookInfo: any} ) {
    ...
    <div className="book__wrapper flex max-[1000px]:gap-4 gap-8 max-[1000px]:flex-col flex-row">
        <div className="book w-full max-[1000px]:order-1">
            <div className="book__title text-[#032b41] mb-4 font-semibold text-[32px]">{bookInfo.title}</div>
            <div className="book__author text-[#032b41] mb-4 font-semibold ">{bookInfo.author}</div>
            <div className="book__sub-title text-[#032b41] mb-4 font-light text-xl">{bookInfo.subTitle}</div>
            <div className="inner__wrapper py-4 mb-6 border-y border-[#e1e7ea]">
                <div className="inner__details--wrapper flex flex-wrap max-w-[400px] gap-y-3">
                    ...
                    text-sm">
                        <div className="inner__icon flex mr-1"><IoIosStarOutline className="w-6 h-6"/></div>
                        <div className="inner__rating">{bookInfo.averageRating} ({bookInfo.totalRating} ratings)</div>
                    </div>
                    <div className="inner__detail flex items-center w-1/2 text-[#032b41] font-medium
                    text-sm">
                    ...
                    <div className="inner__detail flex items-center w-1/2 text-[#032b41] font-medium
                    text-sm">
                        <div className="inner__icon flex mr-1"><IoMicOutline className="w-6 h-6"/></div>
                        <div className="book__type">{bookInfo.type}</div>
                    </div>
                    <div className="inner__detail flex items-center w-1/2 text-[#032b41] font-medium
                    text-sm">
                        <div className="inner__icon flex mr-1"><HiOutlineLightBulb className="w-6 h-6"/></div>
                        <div className="book__key--ideas">{bookInfo.keyIdeas} Key ideas</div>
                    </div>
                </div>
            </div>
            ...
            <div className="book__second--title text-lg text-[#032b41] mb-4 font-semibold">What's it about?</div>
            <div className="book__tags--wrapper flex flex-wrap gap-4 mb-4">
                {bookInfo.tags.map((tag: string, index: number) => (
                    <div key={index} className="book__tag bg-[#f1f6f4] px-4 h-12
                    flex items-center text-[#032b41] cursor-not-allowed font-medium
                    rounded-sm ">{tag}</div>
                ))}
            </div>
            <div className="book__description text-[#032b41] mb-4 leading-[1.5]">{bookInfo.bookDescription}</div>
            <div className="book__about text-lg text-[#032b41] mb-4 font-semibold">About the author</div>
            <div className="book__about--description text-[#032b41] leading-[1.5]">{bookInfo.authorDescription}</div>
        </div>
